...
/**
 * Manages secure storage and retrieval of the LLM API key.
 * Priority order:
 * 1. VS Code SecretStorage (most secure)
 * 2. Environment variable LLM_API_KEY
 * 3. VS Code settings (least secure, not recommended)
 */
export class ApiKeyManager {
    private static readonly SECRET_KEY = 'devcopilot.llmApiKey';
    ...
    async promptAndStoreApiKey(): Promise<boolean> {
        const apiKey = await vscode.window.showInputBox({
            ...
            password: true,
            ignoreFocusOut: true,
            placeHolder: 'gsk_..., sk-..., or custom API key',
            validateInput: (value) => {
                if (!value || !value.trim()) {
    ...
    async getApiKeySource(): Promise<string> {
        ...
        const envKey = process.env.LLM_API_KEY;
        if (envKey && envKey.trim()) {
            return 'Environment Variable (LLM_API_KEY)';
        }

        const config = vscode.workspace.getConfiguration('devcopilotV2');
        const settingsKey = config.get<string>('apiKey', '');
        if (settingsKey && settingsKey.trim()) {
            return 'Settings (Not Recommended)';
        }
