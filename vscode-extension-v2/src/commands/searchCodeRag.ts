...
export async function searchCodeRagCommand(resultsProvider: ResultsViewProvider, stateManager: StateManager) {
    ...
    if (!stateManager.isPipelineConfigured()) {
        ...
        if (action === 'Index Repository') {
            vscode.commands.executeCommand('devcopilotV2.indexRepository');
        }
        return;
    }
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            await vscode.commands.executeCommand('devcopilotV2.resultsView.focus');
