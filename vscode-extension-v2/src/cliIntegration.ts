...
export async function getConfig(stateManager?: StateManager, apiKey?: string): Promise<CLIConfig> {
    const config = vscode.workspace.getConfiguration('devcopilotV2');
    ...
    // API key is now passed in from ApiKeyManager, not read directly
...
export async function executeCLI(args: string[], apiKey?: string): Promise<CLIResult> {
    const config = await getConfig(undefined, apiKey);
...
export async function searchCode(query: string, topK: number = 5, stateManager?: StateManager, apiKey?: string): Promise<any[]> {
    // Legacy function - redirects to HyDE search for backward compatibility
    return searchCodeHyde(query, topK, stateManager, apiKey);
}

export async function searchCodeHyde(query: string, topK: number = 5, stateManager?: StateManager, apiKey?: string): Promise<any[]> {
    const config = await getConfig(stateManager, apiKey);
    
    if (!config.apiKey) {
        throw new Error('API key not configured. Use "DevCopilot v2: Set API Key" command or set LLM_API_KEY environment variable.');
    }
    ...
    const result = await executeCLI(args, config.apiKey);
...
/**
 * Get the ApiKeyManager instance from the extension context.
 * This allows command files to access the API key manager without circular dependencies.
 */
export function getApiKeyManager(): ApiKeyManager | undefined {
    ...
    if (extension && extension.isActive) {
        ...
        if (context) {
            return (context as any).apiKeyManager;
        }
    }
    return undefined;
}
