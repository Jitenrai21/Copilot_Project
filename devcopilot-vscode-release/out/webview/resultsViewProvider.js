...
const simpleMarkdownToHtml_1 = require("./simpleMarkdownToHtml");
class ResultsViewProvider {
    ...
    showRagResults(query, answer, sources) {
    ...
    showPRSummary(summary, repoPath, timeout) {
    ...
    showError(title, message, repoPath, timeout, cliCommand) {
        if (this._view) {
            this._view.show?.(true);
...
<body>
    ...
    _getSearchResultsHtml(query, results) {
...
<body>
    ...
    _getRagResultsHtml(query, answer, sources) {
        // Use the markdown-to-HTML converter for the answer
        const formattedAnswer = (0, simpleMarkdownToHtml_1.simpleMarkdownToHtml)(answer);
...
<body>
    ...
    _getPRSummaryHtml(summary, repoPath, timeout) {
     ...
     */
    _buildFileChangeCards(fileChanges) {
     ...
     */
    _buildPipelineInfoCard(pipelineInfo) {
        if (!pipelineInfo)
            return '';
        return `
...
<body>
    ...
    _getErrorHtml(title, message, repoPath, timeout, cliCommand) {
...
<body>
     ...
     */
    async _retrySummarization(repoPath, timeout) {
        if (!repoPath) {
     ...
     */
    async _showCliCommand(cliCommand) {
        if (!cliCommand) {
            return;
        }
        const action = await vscode.window.showInformationMessage('CLI Command', { modal: true, detail: cliCommand }, 'Copy to Clipboard', 'Close');
...
ResultsViewProvider.viewType = 'devcopilotV3.resultsView';
